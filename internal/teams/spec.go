...
func LoadSpec(data []byte) (*Spec, error) {
	...
	if spec.FrontAgent == "" {
		return nil, fmt.Errorf("team: required field %q is missing", "front_agent")
	}
	if spec.Agents == nil {
	...
	for i, agent := range spec.Agents {
		if agent.ID == "" {
			return nil, fmt.Errorf("team: agent at index %d is missing %q", i, "id")
		}
		if agent.SoulFile == "" {
			return nil, fmt.Errorf("team: agent %q is missing %q", agent.ID, "soul_file")
		}
		if agentIDs[agent.ID] {
			return nil, fmt.Errorf("team: duplicate agent %q", agent.ID)
		}
		agentIDs[agent.ID] = true
	}

	if !agentIDs[spec.FrontAgent] {
		return nil, fmt.Errorf("team: front_agent %q is not declared in agents", spec.FrontAgent)
	}

	for _, agent := range spec.Agents {
		for _, target := range agent.CanSpawn {
			if !agentIDs[target] {
				return nil, fmt.Errorf("team: agent %q can_spawn references undeclared agent %q", agent.ID, target)
			}
		}
		for _, target := range agent.CanMessage {
			if !agentIDs[target] {
				return nil, fmt.Errorf("team: agent %q can_message references undeclared agent %q", agent.ID, target)
			}
		}
	}
