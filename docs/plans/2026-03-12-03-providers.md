...
**Goal:** Build the `internal/providers` package — the `LLMProvider` interface, the `trackingProvider` decorator, and three concrete implementations (OpenAI key, GitHub Copilot gRPC, Codex OAuth PKCE).

**Architecture:** One shared interface (`LLMProvider`) with three implementations living in sub-packages. A decorator (`trackingProvider`) wraps any provider and calls `CostGuard.Track()` automatically on every successful Chat. A factory function (`New`) selects the provider at startup based on `cfg.LLMProvider`. To avoid an import cycle (`providers` ← `providers/{openai,copilot,codex}` ← `providers`), the factory lives in a dedicated `internal/providers/factory` package that sits above all sub-packages. OpenAI is the production-ready path; Copilot uses the real `github.com/github/copilot-sdk/go` v0.1.30 gRPC SDK; Codex uses OAuth2 PKCE against the unofficial chatgpt.com backend with token persistence in SQLite.

**Tech Stack:** Go 1.25, `github.com/openai/openai-go` (OpenAI SDK), `github.com/github/copilot-sdk/go` v0.1.30 (Copilot gRPC SDK), `golang.org/x/oauth2` (PKCE flow), standard `net/http` (Codex backend call), `net/http/httptest` (test mocking)
...
```
Task 1  internal/providers/llm.go                   — interface + types (no New func)
...
Task 4  internal/providers/copilot/copilot.go       — Copilot gRPC SDK implementation
Task 5  internal/providers/codex/codex.go           — Codex OAuth PKCE implementation
Task 6  internal/providers/factory/factory.go       — factory + config OpenAIModel field
```

Tasks 3–5 are independent and can be written in any order after Task 2.

> **Import cycle note:** The factory (`New`) lives in `internal/providers/factory/` — a
> dedicated package sitting above the three sub-packages — to avoid the cycle that would
> arise if `internal/providers/llm.go` imported `providers/{openai,copilot,codex}` while
> those sub-packages import `providers` for the shared types.

---

### Task 1: `internal/providers/llm.go` — interface + types
...
```bash
go get github.com/github/copilot-sdk/go@v0.1.30
go mod tidy
```
...
The test verifies:
1. `Provider` satisfies `LLMProvider` at compile time.
2. `Name()` returns `"copilot"`.
3. `New` accepts a gRPC bridge address.
4. `Chat` returns an error when the bridge is unreachable (not panicking).
...
func TestCopilotChatReturnsErrorWhenBridgeUnreachable(t *testing.T) {
	// Point at a port with nothing listening — should return a connection error.
	p := copilotprovider.New("localhost:19999")
	_, err := p.Chat(context.Background(), []providers.Message{
		{Role: "user", Content: "hello"},
	}, nil)
	if err == nil {
		t.Fatal("expected error when bridge is unreachable, got nil")
	}
}

func TestCopilotDoesNotPanic(t *testing.T) {
	p := copilotprovider.New("localhost:19999")
	defer func() {
		if r := recover(); r != nil {
...
// session with ApproveAll permission handler (headless), encodes the message
// history as a JSON array, sends it via SendAndWait, destroys the session,
// and returns the response. TotalCostUSD is always 0.
...
```bash
git add internal/providers/copilot/ go.mod go.sum
git commit -m "feat: add copilot provider with real github/copilot-sdk/go v0.1.30 gRPC implementation"
```
