<script lang="ts">
	...
	const configuredSurfaces = $derived(
		data.skills.surfaces.filter((surface) => surface.configured || surface.active)
	);
	const credentialSurfaces = $derived(
		data.skills.surfaces.filter((surface) => surface.credential_state !== 'operator_managed')
	);
	const repoTools = $derived(data.skills.tools.filter((tool) => tool.family === 'repo').length);
	...
	<div class="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-6">
		...
		<div class="grid gap-4 xl:grid-cols-4">
			<SurfaceMetricCard
				...
				value={String(data.skills.summary.shipped_surfaces)}
				detail={`${data.skills.summary.configured_surfaces} configured in this runtime.`}
				tone="accent"
			/>
			<SurfaceMetricCard
				label="Installed Tools"
				value={String(data.skills.summary.installed_tools)}
				detail={`${repoTools} repo · ${connectorTools} connector`}
			/>
			<SurfaceMetricCard
				label="Credentials Ready"
				value={String(data.skills.summary.ready_credentials)}
				detail={`${data.skills.summary.missing_credentials} need setup`}
				tone="warning"
			/>
			...
			<div class="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
				<section class="gc-panel-soft px-5 py-5">
					...
					<div class="mt-6 grid gap-4 lg:grid-cols-2">
						...
						<section class="border border-[var(--gc-border)] px-4 py-4">
							...
							{:else}
								<div class="mt-4 grid gap-4">
									{#each data.skills.tools as tool (tool.name)}
										<div
				...
				<section class="gc-panel-soft px-5 py-5">
					...
					<p class="gc-copy mt-3 text-[var(--gc-ink-2)]">
						...
						marketplace or mutate install state outside the shipped config and connector auth flows.
					</p>
				</section>
			</div>
		{:else if activeTab === 'available'}
			<div class="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(18rem,0.9fr)]">
				<section class="gc-panel-soft px-5 py-5">
					...
					{#if data.skills.surfaces.length === 0}
						<p class="gc-copy mt-4 text-[var(--gc-ink-2)]">No shipped surfaces reported.</p>
					{:else}
						<div class="mt-5 grid gap-4 lg:grid-cols-2">
							{#each data.skills.surfaces as surface (surface.id)}
								<SurfaceStatusCard {surface} />
							{/each}
						</div>
					{/if}
				...
				<section class="gc-panel-soft px-5 py-5">
					...
					<h2 class="gc-panel-title mt-3 text-[var(--gc-ink)]">Availability is not installation</h2>
					<p class="gc-copy mt-3 text-[var(--gc-ink-2)]">
		...
		{:else}
			<div class="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(18rem,0.9fr)]">
				<section class="gc-panel-soft px-5 py-5">
					...
					<p class="gc-copy mt-3 text-[var(--gc-ink-2)]">
						Secret-bearing surfaces report readiness here without exposing the secret value itself.
						Missing state tells you setup is still required before that surface can activate
						cleanly.
					</p>
