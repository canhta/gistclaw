<script lang="ts">
	...
	const commitLabel = $derived.by(() =>
		!update
			? ''
			: update.release.commit === 'unknown'
				? 'unknown'
				: update.release.commit.slice(0, 12)
	);
	const warningCount = $derived(update?.storage.warnings.length ?? 0);
	const backupPath = $derived(
		!update || update.storage.latest_backup_path === ''
			? 'No backup path recorded.'
			: update.storage.latest_backup_path
	);
		...
		{#if update === null}
			...
			<div class="mt-2">
				<SurfaceLoadErrorPanel
					label="UPDATE"
					title="Update board unavailable"
					detail="The browser could not load the maintenance status feed from this daemon. Reload to retry."
				/>
			</div>
		{:else}
			{#if updateNotice !== ''}
			...
			<div class="grid gap-4 xl:grid-cols-4">
				<SurfaceMetricCard
					...
					detail={`Commit ${commitLabel} · built ${update.release.build_date_label}`}
					tone="accent"
				/>
				<SurfaceMetricCard
					label="Runtime Uptime"
					value={update.runtime.uptime_label}
					detail={`Started ${update.runtime.started_at_label}`}
				/>
				<SurfaceMetricCard
					label="Backup Status"
					value={update.storage.backup_status}
				...
				<div class="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(18rem,0.85fr)]">
					<section class="gc-panel-soft px-5 py-5">
						...
						<p class="gc-copy mt-3 max-w-2xl text-[var(--gc-ink-2)]">
							...
							build, install, and service context the runtime is using so you can review the
							release, apply it, and bring the daemon back under observation without leaving the
							control UI.
						</p>

						<div class="mt-5 flex flex-wrap gap-3">
							<a
								href={update.guides.release_notes_url}
								rel="external"
								class="gc-action gc-action-solid px-4 py-2"
							>
								GitHub Releases
							</a>
							<span class="gc-action px-4 py-2">{update.guides.changelog_path}</span>
						</div>
							...
							<div class="border border-[var(--gc-border)] px-4 py-4">
								...
								<div class="mt-3 space-y-3">
									<p class="gc-copy text-[var(--gc-ink)]">
										{update.release.version} · {commitLabel}
									</p>
									<p class="gc-machine">{update.release.build_date_label}</p>
								</div>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Restart policy</p>
								<div class="mt-3 space-y-3">
									<p class="gc-copy text-[var(--gc-ink)]">{update.service.restart_policy}</p>
									<p class="gc-machine">{update.install.service_unit_path}</p>
								</div>
							</div>
						</div>
							...
							<div class="border border-[var(--gc-border)] px-4 py-4">
								...
								<div class="mt-3 grid gap-3">
									<div>
										...
										<p class="gc-machine mt-2 break-all">{update.install.config_path}</p>
									</div>
									<div>
										<p class="gc-stamp">State</p>
										<p class="gc-machine mt-2 break-all">{update.install.state_dir}</p>
									</div>
									<div>
										<p class="gc-stamp">Storage root</p>
										<p class="gc-machine mt-2 break-all">{update.install.storage_root}</p>
									</div>
								</div>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								...
								<div class="mt-3 grid gap-3">
									<div>
										...
										<p class="gc-machine mt-2">{update.guides.ubuntu_doc_path}</p>
									</div>
									<div>
										<p class="gc-stamp">macOS</p>
										<p class="gc-machine mt-2">{update.guides.macos_doc_path}</p>
									</div>
									<div>
										<p class="gc-stamp">Recovery</p>
										<p class="gc-machine mt-2">{update.guides.recovery_doc_path}</p>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="mt-6 border border-[var(--gc-border)] px-4 py-4">
							...
							<pre
								class="gc-code mt-4 max-h-[18rem] overflow-auto border border-[var(--gc-border)] bg-[var(--gc-canvas)] px-4 py-4">{update
									.service.unit_preview}</pre>
						</div>
					</section>
						...
						<div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
							<div class="border border-[var(--gc-border)] px-4 py-4">
								...
								<p class="gc-copy mt-3 text-[var(--gc-ink)]">
									{update.runtime.started_at_label}
								</p>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Uptime</p>
								<p class="gc-value mt-3 text-[1.2rem]">{update.runtime.uptime_label}</p>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Pending approvals</p>
								<p class="gc-value mt-3 text-[1.2rem]">{update.runtime.pending_approvals}</p>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Active runs</p>
								<p class="gc-value mt-3 text-[1.2rem]">{update.runtime.active_runs}</p>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Interrupted runs</p>
								<p class="gc-value mt-3 text-[1.2rem]">{update.runtime.interrupted_runs}</p>
							</div>
							<div class="border border-[var(--gc-border)] px-4 py-4">
								<p class="gc-stamp text-[var(--gc-ink-3)]">Backup status</p>
								<p class="gc-copy mt-3 text-[var(--gc-ink)]">{update.storage.backup_status}</p>
							</div>
						</div>
					</section>
						...
						<section class="gc-panel-soft px-5 py-5">
							...
							<div class="mt-4 space-y-4">
								<div>
									...
									{#if update.storage.latest_backup_at_label !== ''}
										<p class="gc-machine mt-2">{update.storage.latest_backup_at_label}</p>
									{/if}
								</div>
								<div class="border-t border-[var(--gc-border)] pt-4">
									<p class="gc-stamp">Storage warnings</p>
									{#if update.storage.warnings.length === 0}
										<p class="gc-copy mt-2 text-[var(--gc-ink-2)]">No storage warnings recorded.</p>
									{:else}
										<div class="mt-3 grid gap-2">
											{#each update.storage.warnings as warning (warning)}
								...
								<div class="border-t border-[var(--gc-border)] pt-4">
									...
									<p class="gc-machine mt-2">
										db={update.storage.database_bytes} wal={update.storage.wal_bytes}
										free={update.storage.free_disk_bytes}
									</p>
								</div>
							</div>
						</section>
