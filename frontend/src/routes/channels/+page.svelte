<script lang="ts">
	...
	const summary = $derived(channels?.summary ?? null);
	const connectors = $derived(channels?.items ?? []);
	const routes = $derived(channels?.routes ?? null);
	const access = $derived(channels?.access ?? null);
	const channelsLoadError = $derived(data.channelsLoadError ?? '');
	const accessLoadError = $derived(data.channelAccessLoadError ?? '');
	const routesLoadError = $derived(data.channelRoutesLoadError ?? '');
	...
	<div class="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-6">
		{#if channels === null}
			{#if channelsLoadError !== ''}
				<SurfaceMessage label="CHANNELS" message={channelsLoadError} className="mb-4" />
			{/if}
			<div class="mt-2">
				<SurfaceLoadErrorPanel
					label="CHANNELS"
					title="Channels board unavailable"
					detail="The browser could not load the channel status board from this daemon. Reload to retry."
				/>
			</div>
		...
		{:else if activeTab === 'login'}
			<div class="mx-auto w-full max-w-6xl">
				{#if accessLoadError !== ''}
					<SurfaceMessage label="ACCESS" message={accessLoadError} className="mb-4" />
				{/if}
				...
				{#if access === null}
					<div class="mt-2">
						<SurfaceLoadErrorPanel
							label="ACCESS"
							title="Channel access unavailable"
							detail="The browser could not load the Telegram and WhatsApp access details from this daemon. Reload to retry."
						/>
					</div>
				{:else}
		...
		{:else}
			<div class="mx-auto w-full max-w-6xl">
				{#if accessLoadError !== ''}
					<SurfaceMessage label="SETTINGS" message={accessLoadError} className="mb-4" />
				{/if}
				{#if routesLoadError !== ''}
					<SurfaceMessage label="ROUTES" message={routesLoadError} className="mb-4" />
				{/if}

				{#if access === null}
					<SurfaceLoadErrorPanel
						label="SETTINGS"
						title="Channel settings unavailable"
						detail="The browser could not load the connector settings details from this daemon. Reload to retry."
					/>
				{:else}
					<section class="gc-panel-soft px-5 py-5">
				...
				{#if routes === null}
					<div class="mt-6">
						<SurfaceLoadErrorPanel
							label="ROUTES"
							title="Route directory unavailable"
							detail="The browser could not load the route directory from this daemon. Reload to retry."
						/>
					</div>
				{:else}
					<section class="gc-panel-soft mt-6 px-5 py-5">
						...
						<form method="GET" class="mt-5 grid gap-4 xl:grid-cols-4">
							...
							<div class="flex flex-col gap-2">
								<label for="route-search" class="gc-stamp text-[var(--gc-ink-3)]"
									>Search routes</label
								>
								<input
									id="route-search"
							...
							<div class="flex flex-col gap-2">
								<label for="route-connector" class="gc-stamp text-[var(--gc-ink-3)]"
									>Connector</label
								>
								<input
									id="route-connector"
							...
							<div class="flex flex-col gap-2">
								<label for="route-status" class="gc-stamp text-[var(--gc-ink-3)]"
									>Route status</label
								>
